...
Items can have value, which is based on the goals that the player can achieve by using the item (e.g. build a tool, build a house, create a potion, etc.). This is analogous to knowledge, which also has value that is largely based on the goals it can help us achieve. For more on this, see: [Item Valuation Network](#1-the-item-valuation-network) below.
...
- throws away garbage items accumulated during the mining process (e.g. [dirt](https://minecraft.gamepedia.com/Dirt) collected when digging). The sleep process relies on [the item valuation network](#1-the-item-valuation-network) to decide which items are garbage and which are valuable and should therefore be kept.
